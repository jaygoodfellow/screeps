...
Job.prototype.perform = function(creep, room) {
	...
	if(creep.name == 'build_16641099') {
		let dropped = creep.room.find(FIND_DROPPED_ENERGY,0)
		if(dropped && _.sum(creep.carry) == 0) creep.pickup(dropped[0])
		let jobStage = this.memory.get(creep, 'jobStage')
...
Job.prototype.employment = function(creep) {
	...
	if(_.isEmpty(tasks) || typeof tasks.steps == 'undefined') {
		...
		// let jobStage = 0
		// if(_.sum(creep.carry) > 0) jobStage = 1
...
// 		result = creep[task.action](target, task.type)
//
// 		if(task.action == 'transfer' && result == ERR_FULL) this.memory.forget(creep, 'tasks')
//
// 		switch (result) {
